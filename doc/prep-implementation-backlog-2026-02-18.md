...
| Core API catalog + mapping service DB-backed | `IN_PROGRESS` | `apps/core-api/src/modules/catalog/*` + `apps/core-api/src/modules/mapping/*` |
...
1. Jalankan smoke test staging untuk endpoint baru EP-010 (`catalog CRUD` + `publish workflow`) dengan admin token runtime.
2. Eksekusi gate `EP-010` di staging (`pnpm --filter @bst/core-api gate:catalog-publish-workflow`) dan simpan evidence final.
3. Lanjutkan hardening UI/UX (komponen form/table/action) agar siap masuk checklist release UI (`EP-013`).
