...
const groupItineraries = (itineraries: any[] | undefined) => {
  if (!itineraries?.length) {
    return []
  }

  const grouped = itineraries.reduce(
    (acc: Record<number, any[]>, itinerary: any) => {
  ...
  return Object.keys(grouped).map((day) => ({
    day: Number(day),
    itineraries: grouped[Number(day)]
  }))
}

export default function TourPageClient({ tour }: Props) {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      setIsVisible(scrollTop + windowHeight < documentHeight - 200)
    }
  ...
  const tourImages = tour?.TourImages?.map((image: any) => image.url) ?? []
  const highlights =
    ...
    <div className="max-w-3xl p-6 mx-auto overflow-hidden bg-white rounded-lg shadow-lg">
      ...
      <div className="mb-6">
        <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-4 gap-1 text-[0.7rem]">
          {tour.duration_days && (
            <div className="flex items-center space-x-2">
              ...
              <div className="p-2">
                ...
                <p className="text-justify text-gray-700">{tour.duration_days}</p>
              </div>
            </div>
          )}
          <div className="flex items-center space-x-2">
          ...
          <div className="flex items-center space-x-2">
            ...
            <div className="p-2">
              <h4 className="font-semibold text-gray-900">Free Entrance Fees</h4>
              <p className="text-justify text-gray-700">Yes</p>
            </div>
          </div>
          <div className="flex items-center space-x-2">
              ...
              <>
                ...
                <ModalBody>
                  <p className="text-justify text-gray-700 text-[0.6rem]">
                    Cancellations requested by customers must be made at least 2
                    days before the tour departure date to receive a 50% refund.
                    Cancellations made less than 2 days before the tour
                    departure date will not be eligible for a refund.
                  </p>
                  <p className="text-justify text-gray-700 text-[0.6rem]">
                    Schedule changes can only be made up to 2 days before the
                    scheduled departure date. Requests for schedule changes made
                    less than 2 days before the departure date will not be
                    accommodated.
                  </p>
                  <p className="text-justify text-gray-700 text-[0.6rem]">
                    PT. Bali Snap Trip reserves the right to cancel tours due to
                    certain reasons such as bad weather, emergencies, or other
                    situations beyond the company&apos;s control. In such cases,
      ...
      {tour.description && (
        <div className="flex flex-col items-start mb-6">
          <h3 className="mb-4 text-2xl font-semibold text-gray-900">Overview</h3>
          <div
            className={`text-sm text-gray-700 leading-relaxed transition-all duration-300 ease-in-out ${isExpanded ? 'max-h-none' : 'max-h-24 overflow-hidden md:max-h-none'}`}
            id="overview-content"
          >
            <div className="text-start">{newLineToBreak(tour.description)}</div>
          </div>
          <button
            aria-controls="overview-content"
            aria-expanded={isExpanded}
            className="text-blue-400 flex items-center space-x-2 cursor-pointer md:hidden"
          ...
          <ul className="text-sm text-justify text-gray-700 list-none">
            {itineraries.map((item: any, index: number) => (
              <li key={`group-${index}`} className="mb-6">
                <div className="my-2 text-lg font-bold">Day {item.day}</div>
                <ul className="text-sm text-justify text-gray-700 list-none">
                  {item.itineraries.map((itinerary: any, itineraryIndex: number) => (
                    <li key={`item-${itineraryIndex}`} className="mb-4">
          ...
          <ul className="text-sm text-gray-700 list-none">
            {tour.TourInclusion.map((item: any, index: number) => (
              <li key={index} className="flex items-start mb-4">
                <FaCheck className="flex-shrink-0 text-green-600" size={16} />
        ...
        <div className="mb-6">
          ...
          <ul className="text-sm text-gray-700 list-none">
            {tour.TourExclusion.map((item: any, index: number) => (
              <li key={index} className="flex items-start mb-4">
                <FaTimes className="flex-shrink-0 text-red-600" size={16} />
                <span className="ml-2 text-start text-[0.8rem]">
      ...
      {tour.duration_days && (
        <div className="flex flex-col items-start mb-6">
          <h3 className="mb-4 text-2xl font-semibold text-gray-900">Duration</h3>
      ...
      <div className="mb-6">
        ...
        <Table removeWrapper aria-labelledby="pricing">
          ...
          <TableBody>
            ...
            <TableRow key="2">
              <TableCell>
                {tour.max_booking + 1} or more adult (10% discount applied)
              </TableCell>
      ...
      <div className="flex justify-center mb-6">
        <FloatingButton data={tour} isFloating={isVisible} />
      </div>
    </div>
  )
}
